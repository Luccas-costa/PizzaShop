...
export default function DashboardButton({
    status, handleStatusProps}: DashboardButtonProps) {
    const [IsAnimation, setIsAnimation] = useState(false);
    const [numberStatus, setNumberStatus] = useState(0); 
    const [IsFinalAnimation, setIsFinalAnimation] = useState(false);
    const [statusButton, setStatusButton] = useState<"Null" | "Deu erro" | "Enviar" | "Enviando" | "Enviado">("Null");
    ...
    return (
                <div
                    ...
                    } ${statusButton === "Enviado" ? "border-zinc-700 cursor-none" : ""}`}
                ...
                >
                    <div className={`${IsAnimation ? "transition-all duration-200 translate-x-[100px] opacity-0" : ""}`}>
                        <ArrowRight
                            size={16}
                            weight='bold'
                            color={status === "Erro" ? "#AF3738" : statusButton === "Enviado" ? "#3f3f46" : "white"}
                        />
                    </div>      
                    <div
                      className={`transition-all duration-200 ${status === "Erro" ? "text-red-500/70" : statusButton === "Enviado" ? "text-zinc-700" : "text-white"} ${IsAnimation ? "opacity-0" : ""} ${IsFinalAnimation ? "opacity-1 " : ""}`}
                    >
                        {statusButton}
                    </div>
                </div>
